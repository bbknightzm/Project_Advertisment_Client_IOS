...
#import "GCTabBar.h"
#define kTabBarHeight 55.0f//65.0f//49.0f

static GCTabBarController *gcTabBarController;

@implementation UIViewController (GCTabBarControllerSupport)

- (GCTabBarController *)gcTabBarController
{
...
@interface GCTabBarController (private)
...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr;
{
	...
	if (self != nil)
	{
		...
		_tabBar = [[GCTabBar alloc] initWithFrame:CGRectMake(0, _containerView.frame.size.height - kTabBarHeight, 320.0f, kTabBarHeight) buttonImages:arr];
		_tabBar.delegate = self;
		
        gcTabBarController = self;
	}
	return self;
}
...
- (void)displayViewAtIndex:(NSUInteger)index
{
	...
	[_transitionView.subviews makeObjectsPerformSelector:@selector(setHidden:) withObject:self];
